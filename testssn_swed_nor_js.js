...
var testSwedFnr = 
            ...
            var $validationField = findMyHiddenBrother($element);
            if (!$number) { $validationField.val(false); console.log('feltet er tomt'); }

            if ($number.indexOf('-') === -1) {
                if ($number.length === 10) {
                    input = $number.slice(0, 6) + "-" + $number.slice(6);
                } else {
                    input = $number.slice(0, 8) + "-" + $number.slice(8);
                }  
            }
            if (!$number.match(/^(\d{2})(\d{2})(\d{2})\-(\d{4})|(\d{4})(\d{2})(\d{2})\-(\d{4})$/)) { $validationField.val(false); };

            // Clean input
            input = $number.replace('-', '');
            if ($number.length === 12) {
                input = $number.substring(2);
            }
